...
class YouTubeAPIError(Exception):
    ...
    YouTube APIの呼び出し時に発生するエラーをラップします。
    """
    pass
...
class YouTubeAPI:
    ...
    YouTube Data API v3の主要機能を簡単に使用できるようにしたクラスです。
    以下の機能を提供します：
    - チャンネル情報の取得
    - 動画情報の取得
    - プレイリストの動画一覧取得
    ...
    def __init__(self, api_key):
        ...
        Args:
            api_key (str): YouTube Data API v3のAPIキー
                         Google Cloud Consoleから取得できます
        
        Raises:
            YouTubeAPIError: APIキーが空またはAPI初期化に失敗した場合
        """
        if not api_key:
            raise YouTubeAPIError("APIキーが必要です")
    ...
    def get_channel_info(self, channel_id):
        ...
        try:
            ...
            request = self.youtube.channels().list(
                part='snippet,statistics',  # 基本情報と統計情報を取得
                id=channel_id
            )
        ...
        try:
            ...
            while len(videos) < max_results:
                request = self.youtube.playlistItems().list(
                    ...
                    maxResults=min(50, max_results - len(videos)),  # API制限に合わせて調整
        ...
        try:
            ...
            while len(comments) < max_results:
                request = self.youtube.commentThreads().list(
                    part='snippet',
                    videoId=video_id,
                    maxResults=min(100, max_results - len(comments)),  # API制限に合わせて調整
                    pageToken=next_page_token,
                    order='time'  # 時系列順で取得
                )
                response = request.execute()
    ...
    def search_all(self, query, search_type='video,channel,playlist', max_results=25, order='relevance'):
        """全体検索
    ...
    def update_caption(self, caption_id, name=None, caption_file=None):
        ...
        try:
            # まず現在の字幕情報を取得
            current_caption = self.youtube.captions().list(
                part='snippet',
                id=caption_id
            ).execute()
    ...
    def upload_channel_banner(self, image_file):
        ...
        """
        try:
            request = self.youtube.channelBanners().insert(
    ...
    def update_channel(self, channel_id, title=None, description=None, keywords=None):
        ...
        try:
            ...
            body = {
                'id': channel_id,
                'snippet': current_channel['snippet']
            }
    ...
    def update_channel_section(self, section_id, title=None, position=None):
        ...
        try:
            # 現在のセクション情報を取得
            current_sections = self.youtube.channelSections().list(
                part='snippet',
                id=section_id
            ).execute()
            ...
            request = self.youtube.channelSections().update(
                part='snippet',
                body=body
            )
            response = request.execute()
    ...
    def post_comment_thread(self, video_id, text, channel_id=None):
        ...
        try:
            ...
            if video_id:
                ...
                raise YouTubeAPIError("video_id または channel_id のいずれかを指定してください")
    ...
    def upload_playlist_image(self, playlist_id, image_file):
        ...
        try:
            request = self.youtube.playlistImages().insert(
                onBehalfOfContentOwner=playlist_id,
                media_body=image_file
            )
            response = request.execute()
            return response
    ...
    def update_playlist_item_position(self, playlist_item_id, new_position):
        ...
        try:
            # 現在のプレイリストアイテム情報を取得
            current_item = self.youtube.playlistItems().list(
                part='snippet',
                id=playlist_item_id
            ).execute()
            
            if not current_item['items']:
                raise YouTubeAPIError(f"プレイリストアイテムが見つかりません: {playlist_item_id}")
    ...
    def set_watermark(self, channel_id, image_file, timing_type='offsetFromStart', offset_ms=15000):
        """チャンネルに透かしを設定
